This repo contains sample reference implementations and example codes

## Scripts to convert labels to Xailient label format

[convert_label_to_xailient_format.py](convert_label_to_xailient_format.py)

[convert_platerecognizer_label_to_xailient_format.py](convert_platerecognizer_label_to_xailient_format.py)

## Scripts for inferencing

[inference_on_video.py](inference_on_video.py)

[http_post_example.py](http_post_example.py)

## Post-processing scripts to improve accuracy

[post_processing_script_to_improved_accuracy.py](post_processing_script_to_improved_accuracy.py)

## Scripts to convert image formats

[png_to_jpg_converter.py](png_to_jpg_converter.py)